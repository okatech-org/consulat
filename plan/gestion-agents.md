...
- [ ] Utiliser le composant DataTable existant (`/src/components/data-table/data-table.tsx`)
- [ ] Afficher : nom, email, téléphone, pays liés, nombre de demandes traitées, temps moyen de traitement, statut
- [ ] Ajouter une recherche et des filtres (pays, spécialisation, statut)
- [ ] Prévoir un état vide (EmptyState) et loading
- [ ] Prévoir pagination et responsive mobile

## 3. Détail d’un agent

- [ ] Créer la page `[id]/page.tsx` pour afficher :
...
- [ ] Utiliser les composants UI existants (stats-card, table, etc.)
- [ ] Prévoir un état vide et loading

## 4. Backend & Server Actions

- [ ] Créer les server actions dans `/src/actions/agents.ts` si besoin (listing, détail, update, etc.)
- [ ] Utiliser Prisma avec les types `BaseAgent`, `FullAgent` (`/src/types/organization.ts`)
- [ ] Gérer la validation et les droits d’accès côté serveur

## 5. Internationalisation

- [ ] Ajouter les clés de traduction dans `/src/i18n/messages/fr/agent.ts` et `/src/i18n/messages/fr/index.ts`
- [ ] Utiliser le hook `useTranslations` dans les composants UI

## 6. UI/UX Mobile-first

- [ ] S’assurer que le listing et la fiche agent sont optimisés pour mobile (table responsive, navigation claire)
- [ ] Utiliser les composants Radix/Shadcn et respecter la charte graphique
...
- `src/app/(authenticated)/dashboard/agents/page.tsx` (listing)
- `src/app/(authenticated)/dashboard/agents/[id]/page.tsx` (détail)
- Server actions dans `src/actions/agents.ts` si besoin
- Ajout de traductions dans `src/i18n/messages/fr/agent.ts`
