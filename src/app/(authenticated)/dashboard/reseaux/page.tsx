      ...
      >
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              { title: 'Nœuds du réseau', value: filteredNodes.length, icon: Network, color: 'blue' },
              { title: 'Connexions totales', value: filteredNodes.reduce((sum, n) => sum + n.connections, 0), icon: Link, color: 'green' },
              { title: 'Influence moyenne', value: Math.round(averageInfluence), icon: TrendingUp, color: 'orange' },
              { title: 'Clusters détectés', value: clusters.length, icon: Target, color: 'red' }
            ].map((stat, index) => (
              <Card 
                key={index}
          ...
          <Card style={{
            ...
            <CardContent>
              <div className="h-[400px] flex items-center justify-center" style={{ background: 'var(--bg-glass-light)', borderRadius: '0.5rem' }}>
                <div className="text-center">
                  ...
                  <Button 
                    ...
                    size="sm" 
                    className="mt-4"
                    onClick={() => toast.info('Graphique réseau', 'Activation de la visualisation interactive')}
                  >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {clusters.map((cluster) => (
                  <div 
                    ...
                    onClick={() => toast.info(`Cluster ${cluster.name}`, `${cluster.nodes} nœuds, ${cluster.connections} connexions`)}
                  >
                    <div className="flex items-start justify-between mb-3">
                      ...
                      <Badge 
                        className={cluster.influence === 'high' ? 'bg-red-500/20 text-red-500' : 'bg-orange-500/20 text-orange-500'}
                      >
                        {cluster.influence}
                      </Badge>
                    </div>
                ...
                {filteredNodes.map((node, index) => (
                  ...
                  >
                    <div 
                      ...
                      style={{
                        background: node.influence >= 90 ? 'linear-gradient(135deg, #ef4444, #dc2626)' :
                                   node.influence >= 80 ? 'linear-gradient(135deg, #f59e0b, #ea580c)' :
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        {node.type === 'profile' ? <Users className="h-4 w-4" /> : <Building2 className="h-4 w-4" />}
                        <h4 className="font-medium text-sm" style={{ color: 'var(--text-primary)' }}>
                          {node.name}
                        </h4>
                        <Badge className={node.influence >= 80 ? 'bg-red-500/20 text-red-500' : 'bg-orange-500/20 text-orange-500'}>
                      ...
                      <div className="flex items-center gap-4 text-xs">
                        ...
                        <Badge className={
                          node.riskLevel === 'high' ? 'bg-red-500/20 text-red-500' :
                          node.riskLevel === 'medium' ? 'bg-yellow-500/20 text-yellow-500' :
