...
const analyticsData = {
  ...
  networkAnalysis: {
    clusters: [
      { id: 'cluster-001', name: 'Réseau Entrepreneurial Paris', size: 124, connections: 89, influence: 'high' },
      { id: 'cluster-002', name: 'Communauté Culturelle Lyon', size: 89, connections: 67, influence: 'medium' },
      { id: 'cluster-003', name: 'Réseau Humanitaire National', size: 156, connections: 134, influence: 'high' },
      { id: 'cluster-004', name: 'Associations Sportives Régionales', size: 67, connections: 45, influence: 'low' }
    ]
  },
  
  predictions: {
    migrationTrends: [
      { period: 'Q1 2025', prediction: 'Augmentation retours Gabon (+23%)', confidence: 85 },
      { period: 'Q2 2025', prediction: 'Stabilisation réseaux entrepreneurs', confidence: 78 },
      { period: 'Q3 2025', prediction: 'Pic d\'activité associations culturelles', confidence: 92 },
      { period: 'Q4 2025', prediction: 'Consolidation réseaux professionnels', confidence: 67 }
    ]
  }
};
...
export default function AnalyticsPage() {
  ...
  const handleClusterAnalysis = (clusterId: string) => {
    toast.info('Analyse de cluster', `Analyse détaillée du cluster ${clusterId} en développement`);
  };
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
            {[
              ...
              { 
                title: 'Confiance IA (%)', 
                value: Math.round(analyticsData.predictions.migrationTrends.reduce((sum, p) => sum + p.confidence, 0) / analyticsData.predictions.migrationTrends.length), 
                icon: Zap, 
                color: 'red',
                change: '+5%'
              }
                  ...
                  <div>
                    ...
                    <div className="space-y-3">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1">
                          <Progress value={analyticsData.demographicAnalysis.genderDistribution.male.percentage} className="h-2" />
                        </div>
                        <div className="w-20 text-right">
                          <div className="text-sm font-bold" style={{ color: 'var(--text-primary)' }}>
                            {analyticsData.demographicAnalysis.genderDistribution.male.count}
                          </div>
                          <div className="text-xs" style={{ color: 'var(--text-muted)' }}>
                            {analyticsData.demographicAnalysis.genderDistribution.male.percentage}%
                          </div>
                        </div>
                      </div>
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1">
                          <Progress value={analyticsData.demographicAnalysis.genderDistribution.female.percentage} className="h-2" />
                        </div>
                        <div className="w-20 text-right">
                          <div className="text-sm font-bold" style={{ color: 'var(--text-primary)' }}>
                            {analyticsData.demographicAnalysis.genderDistribution.female.count}
                          </div>
                          <div className="text-xs" style={{ color: 'var(--text-muted)' }}>
                            {analyticsData.demographicAnalysis.genderDistribution.female.percentage}%
                          </div>
                        </div>
                      </div>
                    </div>
            ...
            >
              ...
              <CardContent>
                <div className="space-y-4">
                  {analyticsData.geographicAnalysis.concentrationZones.map((zone, index) => {
                    const densityColor = zone.density === 'high' ? '#ef4444' : 
                                        zone.density === 'medium' ? '#f59e0b' : '#10b981';
                    ...
                    return (
                      <div 
                        ...
                        onClick={() => toast.info(`Analyse de ${zone.zone}`, 'Analyse détaillée en développement')}
                      >
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-3">
                            <div 
                          ...
                          <div className="flex items-center gap-4">
                            ...
                            <div className="text-center">
                              <div className="text-sm font-bold" style={{ color: 'var(--accent-intel)' }}>
                                {zone.associations}
                              </div>
                              <div className="text-xs" style={{ color: 'var(--text-muted)' }}>
                                associations
              ...
              <CardContent>
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                  {analyticsData.networkAnalysis.clusters.map((cluster) => {
                    const influenceColor = cluster.influence === 'high' ? '#ef4444' : 
                                          cluster.influence === 'medium' ? '#f59e0b' : '#10b981';
                      ...
                      >
                        <div className="flex items-start justify-between mb-3">
                          ...
                          <Badge 
                            className="text-xs"
                            style={{
                              background: cluster.influence === 'high' ? 'rgba(239, 68, 68, 0.2)' :
                                         cluster.influence === 'medium' ? 'rgba(245, 158, 11, 0.2)' :
                                         'rgba(16, 185, 129, 0.2)',
                      ...
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center gap-2">
                          ...
                          <div 
                            ...
                            style={{
                              background: prediction.confidence >= 85 ? '#10b981' :
                                         prediction.confidence >= 70 ? '#f59e0b' : '#ef4444'
                            }}
                          />
                        </div>
                        <div className="text-sm font-medium" style={{ color: 'var(--accent-intel)' }}>
              ...
              <CardContent>
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-3">
                      {analyticsData.riskAnalysis.riskDistribution.map((risk) => (
                        <div key={risk.level} className="flex items-center gap-3">
                          <div className="w-20 text-sm font-medium" style={{ color: 'var(--text-secondary)' }}>
                            {risk.level}
                          </div>
                          <div className="flex-1">
                            <Progress 
                  ...
                  <div>
                    ...
                    <div className="space-y-3">
                      <div className="flex items-center justify-between p-3 rounded-lg" style={{ background: 'var(--bg-glass-light)' }}>
                        <div className="flex items-center gap-2">
                          <TrendingUp className="h-4 w-4 text-red-500" />
                          <span className="text-sm" style={{ color: 'var(--text-primary)' }}>En augmentation</span>
                        </div>
                      ...
                      <div className="flex items-center justify-between p-3 rounded-lg" style={{ background: 'var(--bg-glass-light)' }}>
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm" style={{ color: 'var(--text-primary)' }}>Stables</span>
                        </div>
                      ...
                      <div className="flex items-center justify-between p-3 rounded-lg" style={{ background: 'var(--bg-glass-light)' }}>
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm" style={{ color: 'var(--text-primary)' }}>En diminution</span>
                        </div>
