...
// Type pour les agents de la liste
type AgentListItem = {
  id: string;
  name: string | null;
  email: string | null;
  ...
  linkedCountries: Array<{ id: string; name: string; code: string }>;
      ...
      },
      {
        ...
        cell: ({ row }) =>
          row.original.assignedServices && row.original.assignedServices.length > 0 ? (
            <div className="flex flex-wrap gap-1">
              {row.original.assignedServices.slice(0, 2).map((service) => (
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <div className="text-center">
            <Badge variant="secondary">{row.original._count.assignedRequests}</Badge>
          </div>
        ),
      },
      {
        ...
        cell: ({ row }) => (
          <div className="text-center">{row.original.completedRequests || 0}</div>
        ),
      },
