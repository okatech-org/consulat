...
export default function RapportsPage() {
  ...
  const [reportType, setReportType] = useState<'summary' | 'detailed' | 'geographic' | 'security'>('summary');
  const [exportFormat, setExportFormat] = useState<'pdf' | 'excel' | 'json'>('pdf');
  const [classification, setClassification] = useState<'public' | 'interne' | 'confidentiel' | 'secret'>('confidentiel');
  const [isGenerating, setIsGenerating] = useState(false);
  const [isRefreshing, setIsRefreshing] = useState(false);
  
  const { toasts, removeToast, success, error, info, warning } = useToast();
  const { data: dashboardStats, isLoading: statsLoading, error: statsError } = useIntelligenceDashboardStats(period === 'month' ? 'month' : period === 'week' ? 'week' : 'year');
  ...
  }, [reportType, period, exportFormat, classification, isGenerating, info, success, error]);
  ...
  const handleDownloadReport = useCallback(async (reportId: string, reportTitle: string) => {
  ...
  const handleViewReport = useCallback((reportId: string, reportTitle: string) => {
                ...
                <div 
                  ...
                  style={{
                    background: metric.color === 'blue' ? 'rgba(59, 130, 246, 0.2)' : 
                               ...
                               metric.color === 'purple' ? 'rgba(168, 85, 247, 0.2)' :
                               'rgba(239, 68, 68, 0.2)',
                ...
                {monthlyTrends.map((trend, index) => (
                  <div key={trend.month} className="space-y-2">
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      <div>
                        <div className="text-xs mb-1" style={{ color: 'var(--text-muted)' }}>Profils</div>
                        <Progress value={(trend.profiles / 3000) * 100} className="h-2" />
                      </div>
                      <div>
                        <div className="text-xs mb-1" style={{ color: 'var(--text-muted)' }}>Notes</div>
                        <Progress value={(trend.notes / 200) * 100} className="h-2" />
                      </div>
                      <div>
                        <div className="text-xs mb-1" style={{ color: 'var(--text-muted)' }}>Efficacité</div>
                  ...
                  <div key={index} className="space-y-2">
                    <div className="flex justify-between items-center">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Badge 
                          className={`text-xs ${
                            perf.value >= perf.target ? 'bg-green-500/20 text-green-500' : 
                ...
                return (
                  ...
                  >
                    <div 
                      ...
                      style={{
                        background: typeStyle.bg.replace('bg-', '').includes('blue') ? 'rgba(59, 130, 246, 0.2)' :
                                   typeStyle.bg.replace('bg-', '').includes('green') ? 'rgba(16, 185, 129, 0.2)' :
                                   typeStyle.bg.replace('bg-', '').includes('red') ? 'rgba(239, 68, 68, 0.2)' :
                                   'rgba(107, 114, 128, 0.2)',
                        color: typeStyle.text.replace('text-', '').includes('blue') ? '#3b82f6' :
                               typeStyle.text.replace('text-', '').includes('green') ? '#10b981' :
                               typeStyle.text.replace('text-', '').includes('red') ? '#ef4444' :
                               '#6b7280'
                      }}
                    >
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        >
                          {report.status === 'completed' ? 'TERMINÉ' :
                           report.status === 'processing' ? 'EN COURS' : 'ÉCHEC'}
                        </Badge>
                      </div>
                      ...
                      <div className="flex items-center gap-4 text-xs" style={{ color: 'var(--text-muted)' }}>
                        ...
                        <span className="flex items-center gap-1">
                          ...
                          {format(report.generatedAt, 'dd/MM/yyyy HH:mm', { locale: fr })}
                        </span>
                      </div>
                    </div>
