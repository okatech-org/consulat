...
export default function MovementsMapPage() {
  ...
  const filteredMovements = useMemo(() => {
    return movementsData.filter(movement => {
      ...
      const matchesPeriod = selectedPeriod === 'all' || movement.period.includes(selectedPeriod);
      return matchesSearch && matchesType && matchesPeriod;
    });
  }, [searchTerm, selectedType, selectedPeriod]);

  const totalTravelers = filteredMovements.reduce((sum, m) => sum + m.travelers, 0);
  const internationalMovements = filteredMovements.filter(m => 
    m.destination.city === 'Libreville' || m.destination.city === 'Port-Gentil'
  ).length;
      ...
      >
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              { title: 'Mouvements tracés', value: filteredMovements.length, icon: Route, color: 'blue' },
              { title: 'Voyageurs estimés', value: totalTravelers, icon: Users, color: 'green' },
              { title: 'Vers le Gabon', value: internationalMovements, icon: Plane, color: 'orange' },
              { title: 'En surveillance', value: filteredMovements.filter(m => m.riskLevel !== 'low').length, icon: TrendingUp, color: 'red' }
            ].map((stat, index) => (
          ...
          <Card style={{
            ...
            <CardContent>
              <div className="h-[400px] relative" style={{ background: 'var(--bg-glass-light)', borderRadius: '0.5rem' }}>
                <svg className="w-full h-full" viewBox="0 0 800 400">
                  ...
                  {/* Flèches de flux */}
                  {[
                    { from: {x: 400, y: 150}, to: {x: 200, y: 300}, color: '#ef4444', width: 4 },
                    { from: {x: 450, y: 200}, to: {x: 400, y: 150}, color: '#f59e0b', width: 3 },
                    { from: {x: 350, y: 250}, to: {x: 450, y: 200}, color: '#10b981', width: 2 }
                  ].map((flow, index) => (
                    <g key={index}>
                      <defs>
                        <marker 
                  ...
                  >
                    <div className="flex items-center gap-2">
                      <div 
                        ...
                        style={{
                          background: movement.riskLevel === 'high' ? '#ef4444' :
                                     movement.riskLevel === 'medium' ? '#f59e0b' : '#10b981'
                        }}
                      />
                    ...
                    <div className="flex items-center gap-2 flex-1">
                      ...
                      <ArrowRight className="h-4 w-4" style={{ color: 'var(--accent-intel)' }} />
                      <div className="text-sm font-medium" style={{ color: 'var(--text-primary)' }}>
