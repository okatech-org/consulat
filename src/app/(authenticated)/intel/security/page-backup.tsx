...
export default function SecurityPage() {
  ...
  const securityMetrics = useMemo(() => {
    // Utiliser les vraies stats quand disponibles, sinon simuler de façon réaliste
    const baseAttempts = intelligenceStats?.totalProfiles ? Math.floor(intelligenceStats.totalProfiles * 0.4) : 1247;
    const failureRate = 0.04; // 4% de taux d'échec réaliste
    ...
    return {
      ...
      alertsTriggered: Math.floor(Math.random() * 30) + 15, // Simulation réaliste
      systemUptime: 99.7,
      encryptionLevel: 256
    };
  }, [intelligenceStats]);
  ...
  const recentSecurityEvents = useMemo(() => {
    ...
    // Générer des événements réalistes selon la période
  ...
  const handleRefresh = async () => {
    ...
    try {
      await refetch();
      // Simuler une actualisation des données de sécurité
  ...
  const handleExportLogs = async () => {
    ...
    try {
      ...
      // Générer et télécharger le CSV des événements
  ...
  const handleShowDataAccess = () => {
    toast.info(`${securityMetrics.dataAccess} accès aux données enregistrés dans les dernières ${timeRange === 'hour' ? '1h' : timeRange === 'day' ? '24h' : timeRange === 'week' ? '7j' : '30j'}`);
  };
  ...
  const handleResolveAlert = async (alert: any, index: number) => {
    try {
      // Simuler la résolution d'alerte
          ...
          <DialogContent className="max-w-lg">
            <DialogHeader>
              <DialogTitle className="flex items-center gap-2">
                {selectedEvent && (
                  <>
                    {React.createElement(getEventIcon(selectedEvent.type || 'system'), { className: 'h-5 w-5' })}
              ...
              <div className="py-4 space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="text-sm" style={{ color: 'var(--text-primary)' }}>{selectedEvent.type.toUpperCase()}</p>
                  </div>
                  <div>
