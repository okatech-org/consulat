...
export function useErrorHandler(options: ErrorHandlerOptions = {}) {
  const { translationNamespace = 'errors', onError, logErrors = true } = options;
  ...
  const handleError = useCallback(
    (err: unknown) => {
      ...
      const appError = isAppError(errorObj)
        ...
        : new AppError(errorObj.message, ErrorType.UNKNOWN, { cause: errorObj });
  ...
  const getErrorMessage = useCallback(
    (errorType: ErrorType, field?: string, code?: string) => {
      try {
