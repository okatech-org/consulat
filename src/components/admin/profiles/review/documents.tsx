...
export function ProfileDocuments({ profile }: ProfileDocumentsProps) {
  ...
  const t_review = useTranslations('admin.profiles.review')
              ...
              <div className="space-y-1">
                ...
                {document && (
                  <div className="space-y-1 text-sm text-muted-foreground">
                    {document.issuedAt && (
                      <p>
                        {t_review('documents.issued_at')}: {format(new Date(document.issuedAt), 'PPP', { locale: fr })}
                      </p>
                    )}
                    {document.expiresAt && (
                      <p>
                        {t_review('documents.expires_at')}: {format(new Date(document.expiresAt), 'PPP', { locale: fr })}
                      </p>
                    )}
                  </div>
                )}
