          ...
          <div className="space-y-2">
            {[
              ...
              <button
                ...
                style={{
                  background: item.active ? 'var(--interactive-hover-strong)' : 'transparent',
...
export default function IntelAgentDashboard() {
  ...
  const { data: dashboardStats, isLoading: statsLoading } = useIntelligenceDashboardStats('month');
  
  const { data: profilesData, isLoading: profilesLoading, error: profilesError } = api.profile.getList.useQuery({
    page: 1,
    limit: 10,
  ...
  const { data: mapData, isLoading: mapLoading, error: mapError } = api.intelligence.getProfilesMap.useQuery({
    filters: undefined
  }, {
    staleTime: 5 * 60 * 1000,
    refetchOnWindowFocus: false,
      ...
      <div className="flex min-h-screen relative z-10">
        ...
        >
          ...
          <div className="mb-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
              {[
                ...
                { title: 'Localisations actives', value: stats.locations, change: '+8.3%', icon: MapPin, color: 'green' },
                { title: 'Notes ce mois', value: stats.notes, change: '-2.1%', icon: FileText, color: 'orange' },
                { title: 'Alertes actives', value: stats.alerts, change: '0%', icon: AlertTriangle, color: 'red' }
              ].map((stat, index) => (
                <div
                  key={index}
                ...
                >
                  ...
                  <div className="mt-4 flex items-center gap-2">
                    <TrendingUp className="h-3 w-3" style={{ color: 'var(--accent-intel)' }} />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 mb-8">
            {[
              { title: 'Voir tous les profils', desc: 'Liste complète des profils gabonais', icon: <Users className="h-4 w-4" />, color: 'blue' },
              { title: 'Carte des profils', desc: 'Répartition géographique', icon: <Map className="h-4 w-4" />, color: 'green' },
              { title: 'Notes récentes', desc: 'Derniers renseignements', icon: <Clipboard className="h-4 w-4" />, color: 'orange' },
              { title: 'Audit sécurité', desc: 'Journaux d\'activité', icon: <Lock className="h-4 w-4" />, color: 'red' }
            ].map((action, index) => (
              <div
                key={index}
